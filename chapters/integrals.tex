...
\structure{\textbf{Example}} The function $F(x)=x^2$ is an antiderivative of the function $f(x)=2x$ as $F'(x)=2x$ for all $\mathbb{R}$.
...
\structure{\textbf{Example}} What is the space covered by an object free falling?
...
\structure{\textbf{Example}} To compute $I_{n}=\int{x^ne^x}\,dx$ applying the integration by parts, we have to choose $u=x^n$ y $dv=e^x\,dx$, so $du=nx^{n-1}\,dx$ and $v=e^{x}$, getting
\[
...
\mode<article>{\input{img/integrals/riemann_sums_article}}
\mode<presentation>{\input{img/integrals/riemann_sums}}
\end{center}
\end{frame}
...
\begin{center}
\tikzsetnextfilename{integrals/area_positive_function}
\input{img/integrals/area_negative_function}
\end{center}
\end{frame}
...
\input{img/integrals/area_function}
\end{center}
\end{frame}
